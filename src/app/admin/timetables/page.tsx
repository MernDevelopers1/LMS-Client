...
export default function TimetableListPage() {
  ...
  const { timetables, status, error } = useAppSelector((state) => state.timetable);
  ...
  const handleEdit = (timetable: any) => {
    ...
    router.push(`/admin/timetables/${timetable.id}`);
  };

  const handleDelete = async (timetable: any) => {
    const dayName = DAYS_OF_WEEK[timetable.dayOfWeek] || `Day ${timetable.dayOfWeek}`;
    if (
      !window.confirm(
