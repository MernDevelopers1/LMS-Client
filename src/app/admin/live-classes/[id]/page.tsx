...
export default function EditLiveClassPage() {
  ...
  const handleSubmit = async (values: any) => {
    ...
    const result = await dispatch(updateLiveClass({ id: liveClassId, ...values }));
  ...
  if (status === "loading" && !selectedLiveClass) {
    return <div className="p-8 text-slate-600">Loading live class details...</div>;
  }

  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-slate-900">Edit Live Class</h1>
